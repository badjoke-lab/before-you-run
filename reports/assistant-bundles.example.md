# Before You Run Assistant Bundles Report

Input manifest: downloads/ai-rules/manifest.example.json  
Output directory: downloads/assistant-bundles  
Network access: false  
AI generation: false

## Summary

- Source rules: 1
- Bundles generated: 2

## Generated bundles

### AGENTS.example.md

- Type: agents-md
- Output: downloads/assistant-bundles/AGENTS.example.md
- Source rules: 1

### cursor-rules.example.md

- Type: cursor-rules
- Output: downloads/assistant-bundles/cursor-rules.example.md
- Source rules: 1

## Source rules

- downloads/ai-rules/unknown-repository-basic.md
